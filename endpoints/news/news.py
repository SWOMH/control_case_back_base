...
async def get_news() -> list[NewsResponse]:
    ...
    except NewsIsEmptyException as e:
        raise HTTPException(
...
@router.get('/test', response_model=NewsResponse, status_code=status.HTTP_200_OK)
async def get_news_test():
    """Тестовый запрос"""
    try:
        news = await db_news.get_news_test()
        return news
